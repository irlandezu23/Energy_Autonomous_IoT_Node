...
{
	if(HAL_OK == HAL_UART_Transmit(&huart1, data, *length, TIMEOUT))
	{
		return LMH_OK;
	}
	else
...
{
	HAL_UART_Receive(&huart1, data, exp_length, TIMEOUT);
	*length = huart1.RxXferSize - huart1.RxXferCount;
	return LMH_OK;
	/*else
	{
		return LMH_ERR;
